...
- Phase `blind`: do not read other seats' analyses or claims. Write your own
  analysis to `_Msg/analysis/$CX_SEAT.md`, then `cxam post claim "<one-liner>"`.
- Phase `debate`: verify other seats' concrete claims by RUNNING COMMANDS,
  then `cxam post verify|challenge "..." --ref <evidence>`. If you lose, post
  `cxam post concede "..."` and adopt the winner's framework.
